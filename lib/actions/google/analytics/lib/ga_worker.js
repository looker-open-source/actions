...
class GoogleAnalyticsActionWorker {
    static async fromHubRequest(hubRequest, actionInstance, logger) {
        const userState = await actionInstance.oauthExtractTokensFromStateJson(`${hubRequest.params.state_json}`, hubRequest.webhookId);
    ...
    async getDataSetSelectOptions(accountSummaries) {
        /*
