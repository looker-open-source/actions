...
class GoogleAdsCustomerMatch extends Hub.OAuthAction {
    ...
    async oauthCheck(hubReq) {
        const log = this.makeLogger(hubReq.webhookId);
        if (hubReq.params.state_json) {
            try {
                const tokenPayload = await this.oauthExtractTokensFromStateJson(hubReq.params.state_json, hubReq.webhookId);
                if (tokenPayload && tokenPayload.tokens && tokenPayload.tokens.access_token) {
                    return true;
                }
            }
            catch (e) {
                log("error", "oauthCheck failed", e.toString());
            }
        }
        return false;
    }
