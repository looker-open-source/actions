    ...
    sendOverwriteData(filename, request, drive, sheet) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const files = yield drive.files.list(options).catch((e) => {
                winston.error(`Error listing drives. Error ${e.toString()}`, { webhookId: request.webhookId });
                throw e;
            });
            if (files.data.files === undefined || files.data.files.length === 0) {
                winston.info(`New file: ${filename}`, { webhookId: request.webhookId });
            ...
            const sheets = yield this.retriableSpreadsheetGet(spreadsheetId, sheet, 0, request.webhookId).catch((e) => {
                winston.error(`Error retrieving spreadsheet. Error ${e.toString()}`, { webhookId: request.webhookId });
                throw e;
            });
