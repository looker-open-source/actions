...
const drive_tokens_1 = require("./drive_tokens");
...
class GoogleDriveAction extends Hub.OAuthActionV2 {
        ...
        if (request.params.state_json) {
            try {
                ...
                if (tokenPayload) {
                    ...
                    const encryptedPayload = await this.oauthMaybeEncryptTokens(tokenPayload, request.webhookId);
                    form.state = new Hub.ActionState();
                    form.state.data = JSON.stringify(encryptedPayload.asJson());
                    return form;
                }
            }
            catch (e) {
    ...
    async oauthFetchAccessToken(request) {
        if (request.fetchTokenState) {
            ...
            if (this.validTokens(tokens, request.webhookId)) {
                const tokenPayload = new drive_tokens_1.DriveTokens(tokens, state.redirecturi);
                const encryptedPayload = await this.oauthMaybeEncryptTokens(tokenPayload, request.webhookId);
    ...
    async oauthExtractTokensFromStateJson(stateJson, requestWebhookId) {
        ...
        if (state.cid && state.payload) {
            ...
            tokenPayload = new drive_tokens_1.DriveTokens(state.tokens, state.redirect);
        }
    ...
    async oauthMaybeEncryptTokens(tokenPayload, requestWebhookId) {
        if (process.env.ENCRYPT_PAYLOAD === "true") {
            return Hub.EncryptedPayload.encrypt(tokenPayload, requestWebhookId);
        }
        else {
            return tokenPayload;
        }
    ...
    async oauthDecryptTokens(encryptedPayload, actionCrypto, requestWebhookId) {
        ...
        const tokenPayload = drive_tokens_1.DriveTokens.fromJson(JSON.parse(jsonPayload));
        return tokenPayload;
    }
    async oauthFetchAndStoreInfo(urlParams, redirectUri, statePayload, requestWebhookId) {
        ...
        if (this.validTokens(tokens, requestWebhookId)) {
            const tokenPayload = new drive_tokens_1.DriveTokens(tokens, redirectUri);
            const encryptedPayload = await this.oauthMaybeEncryptTokens(tokenPayload, requestWebhookId);
            await https.post({
