...
class GoogleDriveAction extends Hub.OAuthActionV2 {
        ...
        if (request.params.state_json) {
            try {
                ...
                if (tokenPayload) {
                    ...
                    const encryptedPayload = await this.oauthEncryptTokens(tokenPayload, new Hub.ActionCrypto(), request.webhookId);
                    form.state = new Hub.ActionState();
    ...
    async oauthExtractTokensFromState(state, requestWebhookId) {
    ...
    async oauthEncryptTokens(tokens, actionCrypto, requestWebhookId) {
        if (process.env.ENCRYPT_PAYLOAD === "true") {
            const jsonTokens = JSON.stringify(tokens);
            const encrypted = await actionCrypto.encrypt(jsonTokens).catch((err) => {
    ...
    async oauthDecryptTokens(tokenPayload, actionCrypto, requestWebhookId) {
        const jsonTokens = await actionCrypto.decrypt(tokenPayload.payload).catch((err) => {
    ...
    async oauthFetchAndStoreInfo(urlParams, redirectUri, statePayload, requestWebhookId) {
        ...
        if (this.validTokens(tokens, requestWebhookId)) {
            ...
            const encryptedPayload = await this.oauthEncryptTokens(tokenPayload, new Hub.ActionCrypto(), requestWebhookId);
            await https.post({
