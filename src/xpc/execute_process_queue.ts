        ...
        return this.queue.add(async () => {
            return new Promise<string>((resolve, reject) => {
                ...
                child.on("message", (actionResponse) => {
                    ...
                    if (!succeeded) {
                        winston.warn(`ChildProcess exited with code: ${code}, signal: ${signal}, for webhookId: ${webhookId}`)
                    }
                    if (!child.killed) {
                        child.kill()
                    }
                    ...
                    if (!succeeded) {
                        winston.warn(`ChildProcess closed with code: ${code}, signal: ${signal}, for webhookId: ${webhookId}`)
                    }
                    if (!child.killed) {
                        child.kill()
                    }
