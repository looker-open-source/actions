...
function expectWebhookMatch(request: Hub.ActionRequest, match: {url: string, buffer: Buffer}) {
  const postSpy = sinon.spy((params: any, callback: (err: any, response: any) => void) => {
    ...
    params.body.pipe(concatStream((buffer) => {
      chai.expect(buffer).to.equal(match.buffer)
    }))
  ...
  describe("action", () => {
    ...
    it("sends right body", () => {
      ...
      return expectWebhookMatch(request, {
        url: "http://abc.example.com/",
        buffer: request.attachment.dataJSON,
      })
    })
