...
export class GoogleOAuthHelper {
    ...
    async makeLoginForm(request: Hub.ActionRequest) {
      ...
      if (!request.params.state_url) {
        this.log("error", "Looker did not provide a state_url. Cannot generate login form.")
        const errorForm = new Hub.ActionForm()
        errorForm.error = "Looker did not provide a valid callback URL to complete authentication. Please try reloading the page or resetting the action configuration."
        return errorForm
      }
