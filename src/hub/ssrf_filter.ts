...
const RESTRICTED_IPV4_RANGES: [string, number][] = [
...
export function ssrfSafeLookup(
  hostname: string,
  options: dns.LookupOneOptions | dns.LookupAllOptions | LookupCallback,
  callback?: LookupCallback,
): void {
  const cb = (typeof options === "function" ? options : callback) as LookupCallback
  const opts = (typeof options === "function" ? {} : options) as dns.LookupOneOptions

  dns.lookup(hostname, opts as any, (err: any, address: any, family: any) => {
    if (err) {
      return cb(err, address, family)
    }
    const resolved: string[] = Array.isArray(address)
      ? address.map((entry: dns.LookupAddress) => entry.address)
      : [address]
    const restricted = resolved.find((entry) => isRestrictedAddress(entry))
    if (restricted !== undefined) {
      return cb(
        new Error(
          `SSRF mitigation: refusing to connect to "${hostname}" which resolves to restricted address ${restricted}`,
        ),
        address,
        family,
      )
 ...
 */
export function assertAllowedRequestUrl(rawUrl: string): void {
  let parsed: URL
  try {
